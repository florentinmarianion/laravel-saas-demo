...
<head>
    ...
    <title>Apps for {{ $user->name }}</title>
    ...
    <nav class="bg-gray-900 border-b border-gray-800 px-6 py-4">
        <div class="max-w-4xl mx-auto flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <span class="text-gray-600 text-sm">/ Apps @ {{ $company->name }}</span>
            </div>
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-xl p-8">
            <div class="flex items-center gap-4 mb-6 pb-6 border-b border-gray-800">
                ...
                <div>
                    ...
                    <p class="text-gray-500 text-xs mt-1">Apps access at <span class="text-white">{{ $company->name }}</span></p>
                </div>
            </div>
            ...
            <form method="POST" action="{{ route('apps.user.sync', [$user, $company]) }}">
                ...
                @if($companyApps->isEmpty())
                    <p class="text-gray-500 text-sm">
                        No apps assigned to {{ $company->name }} yet.
                        <a href="{{ route('apps.company', $company) }}" class="text-blue-400 hover:text-blue-300">Assign apps to company first.</a>
                    </p>
                @else
                    <h2 class="text-white font-semibold mb-4">Available Apps at {{ $company->name }}</h2>
